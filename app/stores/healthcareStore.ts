...
import {
	type HealthcareCard,
	type HealthcareDataImport,
	type HealthcareSearchFilters,
	type HealthcareExportOptions,
	type ClaimData,
	type BudgetData,
	type MemberData,
	type ProviderData,
...
} from '@/app/bridges/healthcare/types.js';
import {type ParsedCSVData} from '@/app/components/loaders/CSVLoader';
import {logPhiAccessEvent, persistSecureRecord} from '@/app/lib/phi-client.js';
...
type HealthcareActions = {
	...
	createHealthcareCard: (
		data: ClaimData | BudgetData | MemberData | ProviderData,
		type: string,
		listId: string,
	) => Promise<string>;
	updateHealthcareCard: (
		cardId: string,
		updates: Partial<HealthcareCard>,
	) => Promise<void>;
	...
	searchHealthcareCards: (
		filters: HealthcareSearchFilters,
	) => Promise<HealthcareCard[]>;
...
function transformClaimToCard(
	claim: ClaimData,
	listId: string,
): Omit<HealthcareCard, 'id' | 'createdAt' | 'updatedAt'> {
	return {
		...
		priority:
			claim.amount > 10_000 ? 'high' : claim.amount > 1000 ? 'normal' : 'low',
		...
		healthcare: {
			...
			workflow: {
				...
				reviewLevel: claim.amount > 10_000 ? 'medical_director' : 'standard',
			},
		},
	};
}

function transformBudgetToCard(
	budget: BudgetData,
	listId: string,
): Omit<HealthcareCard, 'id' | 'createdAt' | 'updatedAt'> {
	const variance = budget.actual
		? ((budget.actual - budget.budget) / budget.budget) * 100
		: 0;
...
function getStatusColor(
	status: string,
): 'red' | 'yellow' | 'green' | 'blue' | 'purple' | 'pink' | 'gray' {
	switch (status) {
		case 'approved': {
			return 'green';
		}
	...
	devtools(
		subscribeWithSelector(
			immer((set, get) => ({
				...
				async importCSVData(data, type, options) {
					const importId = `import-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
					...
					try {
						...
						const lists = await get().createListsFromTemplate(
							boardId,
							options.template,
						);
						...
						set((state) => {
							...
							state.importProgress[importId].errors.push(
				...
				async createListsFromTemplate(boardId: string, templateId: string) {
					const boardStore = useBoardStore.getState();
					const lists: string[] = [];
				...
				async transformDataToCards(importId) {
					...
					try {
						for (let i = 0; i < importJob.data.rows.length; i++) {
							...
							if (importJob.type === 'claims') {
								const claimData: ClaimData = {
									...
									serviceType:
										row['serviceType'] || row['service_type'] || 'General',
								...
								const lists = boardStore.getListsByBoard(
									...
									lists.find(
										(l) =>
											...
											(claimData.status === 'pending' &&
												l.title.toLowerCase().includes('pending')),
									) || lists[0];
							...
							if (importJob.type === 'budget') {
								const budgetData: BudgetData = {
									period: row['period'] || '',
									month: row['month'] || '',
									employees: Number.parseInt(row['employees'] || '0'),
									members: Number.parseInt(row['members'] || '0'),
									budget: Number.parseFloat(row['budget'] || '0'),
									actual: Number.parseFloat(row['actual'] || '0'),
									variance: Number.parseFloat(row['variance'] || '0'),
								...
								const variance =
									(((budgetData.actual || 0) - budgetData.budget) /
										budgetData.budget) *
									100;
								const targetList =
									variance > 5
										? lists.find((l) =>
												l.title.toLowerCase().includes('over'),
											) || lists[0]
										: variance < -5
											? lists.find((l) =>
													l.title.toLowerCase().includes('under'),
												) || lists[0]
											: lists[0];
							...
							set((state) => {
								...
								state.importProgress[importId].progress = Math.round(
									((i + 1) / importJob.data.rows.length) * 100,
								);
							});
						}
						...
						set((state) => {
							state.importProgress[importId].status = 'error';
							state.importProgress[importId].errors.push(
								error instanceof Error ? error.message : 'Card creation failed',
							);
						});
						throw error;
					}
				...
				async createHealthcareCard(data, type, listId) {
					const boardStore = useBoardStore.getState();
					let cardData: Omit<HealthcareCard, 'id' | 'createdAt' | 'updatedAt'>;

					if (type === 'claim' && 'claimId' in data) {
						cardData = transformClaimToCard(data, listId);
					} else if (type === 'budget' && 'period' in data) {
						cardData = transformBudgetToCard(data, listId);
					} else {
						throw new Error(`Unsupported data type: ${type}`);
					}

					cardData.boardId = boardStore.activeBoard!;
					return boardStore.createCard(cardData as any);
				},

				async updateHealthcareCard(cardId, updates) {
					const boardStore = useBoardStore.getState();
					...
					if (updates.claimData || updates.memberData) {
						void get().logPHIAccess(cardId, 'modify', 'Card data updated');
					}
				},
				...
				maskPHI(cardId, fields) {
					set((state) => {
						for (const field of fields)
							state.maskedFields.add(`${cardId}:${field}`);
					});
				},
				...
				unmaskPHI(cardId, fields, justification) {
					set((state) => {
						for (const field of fields)
							state.maskedFields.delete(`${cardId}:${field}`);
					});

					void get().logPHIAccess(cardId, 'unmask', justification);
				},

				async runComplianceCheck(cardId) {
				...
				async logPHIAccess(cardId, action, justification) {
					const userId = 'current-user'; // TODO: Pull from auth context
					try {
						const entry = await logPhiAccessEvent({
							resourceId: cardId,
							userId,
							action,
							justification,
						...
						set((state) => {
							state.phiAccessLog.push({
								userId: entry.userId,
								cardId: entry.resourceId,
								action: entry.action,
								timestamp: new Date(entry.timestamp),
								justification: entry.justification ?? justification,
							});
						});
					} catch (error) {
						console.error('Failed to record PHI access event', error);
					}
				},

				async searchHealthcareCards(filters) {
					const boardStore = useBoardStore.getState();
					...
					return allCards.filter((card) => {
						if (filters.claimAmount) {
							...
							if (filters.claimAmount.min && amount < filters.claimAmount.min)
								return false;
							if (filters.claimAmount.max && amount > filters.claimAmount.max)
								return false;
						}
				...
				async exportHealthcareData(options) {
				...
				async createBoardFromTemplate(templateId, title) {
					return get().importCSVData(
				...
				calculateMetrics() {
					...
					set((draft) => {
						draft.metrics = {
							...
							totalBudget: state.budgetData.reduce(
								(sum, b) => sum + b.budget,
								0,
							),
							activeMembers: new Set(state.claimsData.map((c) => c.memberId))
								.size,
							complianceScore:
								state.complianceChecks.length > 0
									? (state.complianceChecks.filter(
											(c) => c.status === 'compliant',
										).length /
											state.complianceChecks.length) *
										100
									: 100,
				...
				async syncWithSecureStorage() {
					try {
						await persistSecureRecord(
							...
							{
								ttlSeconds: 60 * 60,
								metadata: {scope: 'healthcareStore'},
							},
						);
					} catch (error) {
						console.error('Failed to sync with secure record store:', error);
					}
				},
...
// Selectors
export const selectImportProgress =
	(importId: string) => (state: HealthcareState & HealthcareActions) =>
		state.importProgress[importId];

export const selectComplianceStatus = (
	state: HealthcareState & HealthcareActions,
