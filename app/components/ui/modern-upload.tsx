...
const ModernUpload: React.FC<ModernUploadProps> = ({
  ...
  <ModernCard tone={tone} padding="lg" hoverable className={cn("flex flex-col gap-6", cardClassName)}>
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div className="space-y-2">
        <h3 className="text-2xl font-semibold text-[var(--foreground)]">{title}</h3>
        {description ? (
          <p className="text-sm leading-relaxed text-[var(--foreground-muted)] max-w-xl">{description}</p>
        ) : null}
      </div>
      <motion.span
    ...
    <CSVLoader
      {...loaderProps}
      className={cn(
        "rounded-2xl border border-dashed border-[var(--surface-border)] bg-[var(--surface)]/90 p-0 shadow-none transition-all duration-300 hover:border-[var(--accent)]",
        loaderClassName,
      )}
    />
