...
import HealthcareAnalyticsBoard from './HealthcareAnalyticsBoard.js';
import {GlassCard} from '@/app/components/ui/glass-card';
import {Textarea} from '@/app/components/ui/textarea';
import {
        persistSecureRecord,
        retrieveSanitizedRecord,
} from '@/app/lib/phi-client.js';

type Row = Record<string, unknown>;

type PlanPerformanceTilesProperties = {
	data: Row[];
	claims?: Row[];
...
function number_(value: unknown): number {
	...
	const parsed = Number.parseFloat(String(value).replaceAll(/[$,\s]/g, ''));
...
function computeMetrics(rows: Row[]): Metrics {
	return rows.reduce<Metrics>(
		(accumulator, row) => {
			const budget = number_(row['Budget'] ?? row['Computed Budget']);
			const medical = number_(
				row['Medical Claims'] ?? row['medical_claims'] ?? row['Medical'],
			);
			const pharmacy = number_(
				row['Pharmacy Claims'] ?? row['pharmacy_claims'] ?? row['Rx'],
			);
			const admin = number_(row['Admin Fees'] ?? row['admin_fees']);
			const stopLossFees = number_(
				row['Stop Loss Premium'] ?? row['stop_loss_premium'],
			);
			const reimburse = number_(
				row['Stop Loss Reimbursements'] ?? row['stop_loss_reimb'],
			);
			const rebates = number_(row['Rx Rebates'] ?? row['pharmacy_rebates']);
			const members = number_(
				row['Member Count'] ?? row['Enrollment'] ?? row['members'],
			);
			const employees = number_(row['Employee Count'] ?? row['employees']);

			accumulator.budget += budget;
			accumulator.medical += medical;
			accumulator.pharmacy += pharmacy;
			accumulator.admin += admin;
			accumulator.stopLossFees += stopLossFees;
			accumulator.reimburse += reimburse;
			accumulator.rebates += rebates;
			accumulator.members += members;
			accumulator.employees += employees;
...
function enrichMetrics(rows: Row[], metrics: Metrics): Metrics {
	const netPaidClaims =
		metrics.medical + metrics.pharmacy - metrics.reimburse - metrics.rebates;
	const totalPlanCost = netPaidClaims + metrics.admin + metrics.stopLossFees;
	const surplus = metrics.budget - totalPlanCost;
	const pctOfBudget =
		metrics.budget > 0 ? (totalPlanCost / metrics.budget) * 100 : 0;
...
function computeMonthlySeries(rows: Row[]): MonthlySeries {
	const months = rows.map((row) =>
		String(row.month ?? row.Month ?? row.period ?? row.Period ?? ''),
	);
	const budget = rows.map((row) =>
		number_(row['Budget'] ?? row['Computed Budget']),
	);
...
export default function PlanPerformanceTiles({
	data,
	claims = [],
	commentaryTitle = 'Commentary',
}: PlanPerformanceTilesProperties) {
	const [selectedTimeframe, setSelectedTimeframe] =
		useState<TimeframeKey>('12M');
	...
	const timeframeMap = useMemo(() => {
		const map = new Map<
			TimeframeKey,
			{rows: Row[]; metrics: Metrics; series: MonthlySeries}
		>();
	...
	const current =
		timeframeMap.get(selectedTimeframe) ?? timeframeMap.get('12M');
	const currentMetrics =
		current?.metrics ?? enrichMetrics(data, computeMetrics(data));
	const currentSeries = current?.series ?? computeMonthlySeries(data);

	const gaugeTimeframes = useMemo(
		() =>
	...
	useEffect(() => {
		...
		const hydrate = async () => {
			try {
				const stored = await retrieveSanitizedRecord<{text?: string}>(
					'planCommentary',
				);
				if (!cancelled && stored?.sanitized?.text) {
					setCommentary(stored.sanitized.text);
					return;
				}
			} catch {
				// Fall back to generated commentary
			}

			if (!cancelled) {
				const baselineValue =
					gaugeTimeframes.find((tf) => tf.key === '12M')?.value ?? 0;
				setCommentary(
					`${commentaryTitle} is running at ${baselineValue.toFixed(1)}% of budget over the rolling period.\n\nHighlight notable claim activity or utilisation changes here.`,
				);
			}
		};
			...
			try {
				...
				await persistSecureRecord(
					...
					},
					{
						...
						metadata: {component: 'PlanPerformanceTiles'},
					},
				);
				setSaveState('saved');
				setTimeout(() => {
	...
	const actualSeries =
		viewMode === 'planCost' ? currentSeries.actual : currentSeries.paidClaims;
	const chartTitle = viewMode === 'planCost' ? 'Net plan cost' : 'Paid claims';

	const monthLegend =
		currentSeries.months.length > 0
			? `Last ${currentSeries.months.length} months`
			: '';
		...
		() => ({
			...
			tooltip: {
				...
				formatter(parameters: any[]) {
					...
					const [actual, budget] = parameters;
	...
	const shareOption = useMemo(
		() => ({
			...
			tooltip: {
				trigger: 'item',
				formatter: (parameters: any) =>
					`${parameters.seriesName}: ${parameters.value.toFixed(1)}%`,
	...
	const summaryBlocks = useMemo(
		() => [
			...
			},
			{
				...
				accent:
					currentMetrics.surplus >= 0 ? 'text-emerald-600' : 'text-rose-600',
			},
			{
			...
			},
			{
				...
				value: formatCurrency(
					currentMetrics.admin + currentMetrics.stopLossFees,
				),
			},
			{
				label: 'Members covered',
				value: numberFormatter.format(
					currentMetrics.members || currentMetrics.employees,
				),
			},
		],
		...
		<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
			<PerformanceIndicator
				...
				onTimeframeChange={(key) => {
					setSelectedTimeframe(key as TimeframeKey);
				}}
			/>
				...
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div>
						<h3 className="text-base font-semibold text-[var(--foreground)]">
							Rolling {selectedTimeframe === '6M' ? '6' : '12'} month trend
						</h3>
						<p className="text-xs text-[var(--foreground-subtle)]">
							{chartTitle} compared with budgeted premium
						</p>
					</div>
			...
			<GlassCard variant="elevated" className="p-5">
				...
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
					{summaryBlocks.map((block) => (
						<div
							key={block.label}
							className="rounded-xl bg-[var(--surface-muted)]/70 p-3"
						>
							<p className="text-xs uppercase tracking-[0.25em] text-[var(--foreground-subtle)]">
								{block.label}
							</p>
							<p
								className={`mt-2 text-lg font-semibold text-right text-[var(--foreground)] ${block.accent ?? ''}`}
							>
								{block.value}
							</p>
							{block.helper ? (
				...
				<div className="mt-3 flex items-center justify-between text-xs text-[var(--foreground-muted)]">
					<div className="flex items-center gap-2">
						<span
							...
							style={{backgroundColor: chartPalette.medical}}
						/>
						<span>Medical</span>
					</div>
					<div className="flex items-center gap-2">
						<span
							className="inline-flex h-2.5 w-2.5 rounded-full"
							style={{backgroundColor: chartPalette.pharmacy}}
						/>
						<span>Pharmacy</span>
					</div>
				</div>
			...
			<GlassCard variant="elevated" className="p-5 lg:col-span-3">
				<button
					type="button"
					onClick={() => {
						setCommentaryOpen((previous) => !previous);
					}}
							...
							<div className="mt-4 space-y-3">
								...
								<div className="flex items-center justify-between text-xs text-[var(--foreground-subtle)]">
									...
									<span className="text-[var(--foreground-subtle)]">
										Use natural language. Avoid repeating figures already shown
										above.
									</span>
								</div>
							</div>
			...
			<HealthcareAnalyticsBoard
				budgetRows={current?.rows ?? data}
				claimsRows={claims}
			/>
		</div>
	);
